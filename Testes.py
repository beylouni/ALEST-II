...
class Jogo:
    def __init__(self, file_path):
        ...
        self.macacos = {m['id']: Macaco(m['id'], m['par'], m['impar'], m['cocos']) for m in infos.values()}
    ...
    def extract(self, file_path):
        ...
        for match_monkey in re.finditer(macaco_r, text):
            ...
            cocos = []
            if cocos_str:
                cocos = re.findall(r'\d+', cocos_str)
    ...
    def play_round(self):
            for i, remetente in self.macacos.values():
                for coco in remetente.cocos:
                    if coco % 2 == 0:
    ...
    def get_winner(self):
        winner = max(self.macacos.values(), key=lambda monkey: len(monkey.cocos))
        return winner.id
    

start_time = time.time()

bingo = Jogo(r"C:\Users\LucianoBeylouni\Downloads\T1_ALEST\casos_t1_2023_1\caso01.txt")
